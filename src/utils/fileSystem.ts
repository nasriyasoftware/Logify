...
class FileSystemManager {
    ...
    private _logLocation = path.join(process.cwd(), 'logs');

    constructor() {
        this._eventEmitter.on('LogifyLogLocationChange', async (newLocation: string) => {
            console.log('Log location event has been recieved to:', newLocation);
            const hasAccess = await this._utils.hasRWAccess(newLocation);
            if (hasAccess) { this._logLocation = newLocation }
        })
    }

    private readonly _utils = {
        hasRWAccess: (location: string): Promise<boolean> => {
        ...
        checkTodaysFolder: () => {
            const { year, month, date } = this._utils.getTodaysLogFileName('Object') as DateParts;
        ...
        getTodaysLogFileName: (as?: 'String' | 'Object'): string | DateParts => {
            const now = new Date();
    ...
    write(chunk: string, stream: WriteStream) {
        try {
